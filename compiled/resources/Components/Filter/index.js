...
class Filter extends react_1.default.Component {
    ...
    // }
    glitch(el) {
        return (<div className={this.state.glitchOn ? 'glitch glitchOn' : 'glitch'}>
        {[...Array(10).keys()].map((i) => (<div key={i + 'hg'} className='line'>
            {el}
          </div>))}
    ...
    render() {
        ...
        <div className='filterWrap'>
          <div className='filterIcon'>{svg_1.default.search(18)}</div>
          <input className='filterInput' spellCheck={false} tabIndex={-1} onChange={(e) => this.props.onInput(e.target.value)}/>
        </div>
        {buttonActionName ? (<div className='filterButton' onClick={() => {
                    this.setState({ glitchOn: false });
                    if (buttonAction)
                        buttonAction();
                }} onMouseEnter={() => this.setState({ glitchOn: true })} onMouseOver={() => this.setState({ glitchOn: true })} onMouseLeave={() => this.setState({ glitchOn: false })}>
            {this.glitch(<div className='filterButtonInner'>
                {'+'}
                {svg_1.default.chain(14)}
              </div>)}
          </div>) : null}
      </div>);
    }
}
exports.default = react_restore_1.default.connect(Filter);
