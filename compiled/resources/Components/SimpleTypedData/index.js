...
exports.SimpleTypedData = void 0;
const SimpleJSON = ({ json }) => {
    return (<div className='simpleJson'>
      {Object.keys(json).map((key, o) => {
            const value = json[key];
            return (<div key={key + o} className='simpleJsonChild'>
...
const SimpleTypedDataInner = ({ typedData }) => typedData.domain ? (<div className='signTypedDataInner'>
    ...
    </div>) : (<div className='signTypedDataSection'>
      <SimpleJSON json={typedData.reduce((data, elem) => {
        data[elem.name] = elem.value;
        return data;
    }, {})}/>
    </div>);
const SimpleTypedData = ({ req }) => {
    const type = req.type;
    const typedData = req.typedMessage.data || {};
    return type === 'signTypedData' || type === 'signErc20Permit' ? (<div className='accountViewScroll cardShow'>
      <div className='txViewData'>
        <div className='txViewDataHeader'>{'Raw Typed Data'}</div>
        <SimpleTypedDataInner {...{ typedData }}/>
      </div>
    </div>) : (<div className='unknownType'>{'Unknown: ' + req.type}</div>);
};
exports.SimpleTypedData = SimpleTypedData;
