...
const RingIcon_1 = __importDefault(require("../../../resources/Components/RingIcon"));
const Cluster_1 = require("../../../resources/Components/Cluster");
const link_1 = __importDefault(require("../../../resources/link"));
const svg_1 = __importDefault(require("../../../resources/svg"));
class _RequestItem extends react_1.default.Component {
    ...
    render() {
        ...
        <Cluster_1.ClusterValue onClick={!headerMode
                ? () => {
                    const crumb = {
                        view: 'requestView',
                        data: {
                    ...
                    link_1.default.send('nav:forward', 'panel', crumb);
                }
                : null}>
          <div key={req.handlerId} className={headerMode ? 'requestItem requestItemHeader' : 'requestItem'}>
            <div className='requestItemBackground' style={{
            ...
            <div className='requestItemTitle'>
              <div className='requestItemTitleLeft'>
                <div className='requestItemIcon'>
                  <RingIcon_1.default color={color} svgName={svgName} img={img} small={true}/>
                </div>
              ...
              <div className='requestItemTitleTime'>
                {this.state.ago === 'NEW' ? (<div className='requestItemTitleTimeItem' style={{ color: 'var(--good)' }}>
                    {this.state.ago}
                  </div>) : (<div className='requestItemTitleTimeItem'>{this.state.ago}</div>)}
              </div>
              <div className={requestItemDetailsClass}>
                <div className={inactive ? 'requestItemWave requestItemWaveDisabled' : 'requestItemWave'}>
                  <div className='requestItemLine'>{svg_1.default.sine()}</div>
                  <div className='requestItemLine requestItemLineShadow'>{svg_1.default.sine()}</div>
                </div>
              </div>
            </div>
            <div style={headerMode ? { pointerEvents: 'auto' } : { pointerEvents: 'none' }}>{children}</div>
            {notice && notice !== status && (<div role='alert' className={requestItemNoticeClass} style={notice === 'see signer' ? { color: 'var(--good)' } : {}}>
                {notice}
              </div>)}
          </div>
