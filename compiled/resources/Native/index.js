...
class Title extends react_1.default.Component {
    handleClose() {
        link_1.default.send('frame:close');
    }
    handleMin() {
        link_1.default.send('frame:min');
    }
    handleMax() {
        link_1.default.send('frame:max');
    }
    handleUnmax() {
        link_1.default.send('frame:unmax');
    }
    handleFull() {
        link_1.default.send('frame:full');
    }
    handleUnfull() {
        link_1.default.send('frame:unfull');
    }
    render() {
        const platform = this.store('platform');
        const { fullscreen, maximized } = this.store('main.frames', window.frameId);
        return (<div className='nativeControls'>
        {platform === 'darwin' ? (<>
            ...
            <div className='windowsControls'>
              ...
              {maximized || fullscreen ? (<div className='windowsControlsButton' onClick={this.handleUnmax}>
                ...
                </div>) : (<div className='windowsControlsButton' onClick={this.handleMax}>
            ...
            <div className='linuxControls'>
              ...
              {maximized || fullscreen ? (<div className='linuxControlsButton' onClick={this.handleUnmax}>
                  <svg className='linuxControlsMax' width='11' height='11' viewBox='0 0 11 11'>
                    <path d='m11 8.7978h-2.2021v2.2022h-8.7979v-8.7978h2.2021v-2.2022h8.7979zm-3.2979-5.5h-6.6012v6.6011h6.6012zm2.1968-2.1968h-6.6012v1.1011h5.5v5.5h1.1011z'/>
                  </svg>
                </div>) : (<div className='linuxControlsButton' onClick={this.handleMax}>
...
exports.default = react_restore_1.default.connect(Title);
