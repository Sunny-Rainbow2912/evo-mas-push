    ...
    async addKeystore(keystore, keystorePassword, password, cb) {
        ...
        try {
            if (keystore.version === 1)
                wallet = await fromV1(keystore, keystorePassword);
            else if (keystore.version === 3)
                wallet = await fromV3(keystore, keystorePassword);
            else
                return cb(new Error('Invalid keystore version'));
        }
        catch (e) {
            return cb(e);
        }
