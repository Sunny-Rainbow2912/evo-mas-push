...
module.exports = {
    ...
    createFromSeed: (signers, seed, password, cb) => {
        if (!seed)
            return cb(new Error('Seed required to create hot signer'));
        if (!password)
            return cb(new Error('Password required to create hot signer'));
    ...
    createFromPhrase: (signers, phrase, password, cb) => {
        if (!phrase)
            return cb(new Error('Phrase required to create hot signer'));
        if (!password)
            return cb(new Error('Password required to create hot signer'));
        if (password.length < 12)
            return cb(new Error('Hot account password is too short'));
    ...
    createFromKeystore: (signers, keystore, keystorePassword, password, cb) => {
        if (!keystore)
            return cb(new Error('Keystore required'));
        if (!keystorePassword)
            return cb(new Error('Keystore password required'));
        if (!password)
            return cb(new Error('Password required to create hot signer'));
        if (password.length < 12)
            return cb(new Error('Hot account password is too short'));
