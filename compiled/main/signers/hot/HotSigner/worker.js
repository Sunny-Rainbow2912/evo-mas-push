...
class HotSignerWorker {
    ...
    handleMessage({ id, method, params, token }) {
        ...
        if (this[method])
            return this[method](params, pseudoCallback);
        // Else return error
    ...
    verifyAddress({ index, address }, pseudoCallback) {
        ...
        this.signMessage({ index, message }, (err, signedMessage) => {
            ...
            if (err)
                return pseudoCallback(err);
            // Signature -> buffer
            const signature = Buffer.from(signedMessage.replace('0x', ''), 'hex');
            // Ensure correct length
            if (signature.length !== 65)
                return pseudoCallback(new Error('EvoTradeWallet verifyAddress signature has incorrect length'));
            // Verify address
            let v = signature[64];
