...
const rpc = {
    ...
    async locateKeystore(cb) {
        try {
            const file = await (0, dialog_1.openFileDialog)();
                ...
                fs.readFile(keystore.filePaths[0], 'utf8', (err, data) => {
                    ...
                    try {
                        ...
                        if (typeof parsed.version !== 'number')
                            cb('Invalid keystore file');
                        if (![1, 3].includes(parsed.version))
                            cb('Invalid keystore version');
                        cb(null, parsed);
                    }
                    catch (err) {
                        cb(err);
    ...
    async resolveEnsName(name, cb) {
        ...
        try {
            const { addresses: { eth: ethAddress } } = await nebula.ens.resolve(name, { timeout: 8000 });
            cb(null, ethAddress);
        }
        catch (err) {
    ...
    openExplorer(chain) {
        if (store('main.mute.explorerWarning')) {
            (0, window_1.openBlockExplorer)(chain);
        }
        else {
