...
class ChainConnection extends EventEmitter {
    ...
    async txEstimates(type, id, gasPrice, currentSymbol, provider) {
        ...
        if (chainUsesOptimismFees(id) && !isTestnet) {
            estimates = await Promise.all(sampleEstimates.map(async ({ label, txExample }) => {
                const tx = {
                    ...txExample,
                    type: 2,
                    chainId: id
            ...
            try {
                ...
                if (provider.connected) {
                    ...
                    const estimatedGasPrice = feeMarket
                        ...
                        : BigNumber(gasPrice);
                    this.feeEstimatesUSD(parseInt(this.chainId), estimatedGasPrice, new Web3Provider(provider)).then((samples) => {
