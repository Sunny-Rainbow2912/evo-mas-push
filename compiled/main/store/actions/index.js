...
module.exports = {
    ...
    syncPath: (u, path, value) => {
        if (!path || path === '*' || path.startsWith('main'))
            return; // Don't allow updates to main state
        u(path, () => value);
    },
    ...
    updateAccount: (u, updatedAccount) => {
        ...
        if (name && !(0, account_1.isDefaultAccountName)({ ...updatedAccount, name })) {
            const accountMetaId = (0, uuid_1.v5)(id, account_1.accountNS);
        ...
        try {
            ...
            u('main', (main) => {
                if (!main.networks[net.type])
                    main.networks[net.type] = {};
                if (main.networks[net.type][net.id])
                    return main; // Network already exists, don't overwrite, notify user
    ...
    setDappStorage: (u, hash, state) => {
        if (state)
            u(`main.dapp.storage.${hash}`, () => state);
    },
    ...
    setBlockHeight: (u, chainId, blockHeight) => {
        u('main.networksMeta.ethereum', (chainsMeta) => {
            if (chainsMeta[chainId]) {
                ...
                electron_log_1.default.error(`Action Error: setBlockHeight chainId: ${chainId} not found in chainsMeta`);
            }
            return chainsMeta;
        });
    },
    setChainColor: (u, chainId, color) => {
        u('main.networksMeta.ethereum', (chainsMeta) => {
            if (chainsMeta[chainId]) {
                ...
                electron_log_1.default.error(`Action Error: setChainColor chainId: ${chainId} not found in chainsMeta`);
            }
            return chainsMeta;
        });
    },
    ...
    removeBalance: (u, chainId, address) => {
        u('main.balances', (balances = {}) => {
            ...
            for (const accountAddress in balances) {
                const balanceIndex = balances[accountAddress].findIndex((balance) => balance.chainId === chainId && balance.address.toLowerCase() === key);
                if (balanceIndex > -1) {
    ...
    removeBalances: (u, address, tokensToRemove) => {
        const needsRemoval = (balance) => tokensToRemove.has((0, balance_1.toTokenId)(balance));
        ...
        u('main.balances', (balances) => {
            ...
            Object.values(balances).forEach((accountBalances) => {
                tokens.forEach((token) => {
                    ...
                    const matchingBalance = accountBalances.find((b) => b.address.toLowerCase() === tokenAddress && b.chainId === token.chainId);
                    if (matchingBalance) {
    ...
    removeCustomTokens: (u, tokens) => {
        const tokenIds = new Set(tokens.map(balance_1.toTokenId));
        const needsRemoval = (token) => tokenIds.has((0, balance_1.toTokenId)(token));
        u('main.tokens.custom', (existing) => {
    ...
    removeKnownTokens: (u, address, tokensToRemove) => {
        const needsRemoval = (token) => tokensToRemove.has((0, balance_1.toTokenId)(token));
    ...
    navForward: (u, windowId, crumb) => {
        if (!windowId || !crumb)
            return electron_log_1.default.warn('Invalid nav forward', windowId, crumb);
    ...
    navBack: (u, windowId, numSteps = 1) => {
        if (!windowId)
            return electron_log_1.default.warn('Invalid nav back', windowId);
        u('windows', windowId, 'nav', (nav) => {
    ...
    updateTypedDataRequest: (u, account, reqId, data) => {
        u('main.accounts', account, 'requests', (requests) => {
            if (!requests[reqId]?.typedMessage?.data) {
                electron_log_1.default.error('No typed data request found for ', { reqId });
                return requests;
            }
