...
const latestStateVersion = () => {
    ...
    const versions = Object.keys(state.__)
        .filter((v) => parseInt(v) <= migrate_1.default.latest)
...
const mainState = {
    ...
    instanceId: main('instanceId', (0, uuid_1.v4)()),
...
Object.keys(initial.main.accounts).forEach((id) => {
    ...
    if (permissions)
        delete permissions[(0, uuid_1.v5)('Unknown', uuid_1.v5.DNS)];
...
initial.main.origins = Object.entries(initial.main.origins).reduce((origins, [id, origin]) => {
    if (id !== (0, uuid_1.v5)('Unknown', uuid_1.v5.DNS)) {
...
initial.main.knownExtensions = Object.fromEntries(Object.entries(initial.main.knownExtensions).filter(([_id, allowed]) => allowed));
initial.main.dapps = Object.fromEntries(Object.entries(initial.main.dapps).map(([id, dapp]) => [
    id,
    { ...dapp, openWhenReady: false }
]));
// ---
function default_1() {
    const migratedState = migrate_1.default.apply(initial);
    const result = StateSchema.safeParse(migratedState);
    if (!result.success) {
        const issues = result.error.issues;
        electron_log_1.default.warn(`Found ${issues.length} issues while parsing saved state`, issues);
    }
