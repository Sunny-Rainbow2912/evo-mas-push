...
function removeGoerliPylonPreset(connection) {
    // remove Goerli Pylon preset
    const isPylon = connection.current === 'pylon';
    if (isPylon) {
        electron_log_1.default.info('Migration 41: removing Pylon presets from Goerli');
    }
    return {
        ...connection,
...
function removeBaseGoerliConnection(connection) {
    // remove Base Goerli Pylon preset
    const isPylon = connection.current === 'pylon';
    if (isPylon) {
        electron_log_1.default.info('Migration 41: removing Pylon presets from Base Goerli');
    }
    return {
        ...connection,
...
const migrate = (initial) => {
    try {
        ...
        electron_log_1.default.error('Migration 41: could not parse state', e);
    }
    return initial;
};
exports.default = {
