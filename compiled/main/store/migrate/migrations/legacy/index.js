...
const migrations = {
        ...
        if (connection) {
            ...
            if (connection.local && connection.local.settings) {
                Object.keys(connection.local.settings).forEach((id) => {
                    if (connection.secondary.settings[id] &&
                        ...
                        if (current === 'direct')
                            current = 'local';
                        if (current)
                            initial.main.networks.ethereum[id].connection.primary.current = current;
                    }
                });
            }
                ...
                Object.keys(connection.secondary.settings).forEach((id) => {
                    if (connection.secondary.settings[id] &&
                        ...
                        if (current === 'direct')
                            current = 'local';
                        if (current)
                            initial.main.networks.ethereum[id].connection.secondary.current = current;
                    }
                });
            }
    ...
    7: (initial) => {
        ...
        Object.keys(addresses).forEach((address) => {
            ...
            const hasPermissions = addresses[address] &&
            ...
            if (!hasPermissions)
                return electron_log_1.default.info(`Address ${address} did not have any permissions or tokens`);
        ...
        Object.keys(initial.main.accounts).forEach((account) => {
            try {
                ...
                if (block.startsWith('0x'))
                    block = parseInt(block, 'hex');
                if (block > 12726312)
                    block = 12726312;
                initial.main.accounts[account].created = block + ':' + localTime;
            }
            catch (e) {
                electron_log_1.default.error('Migration error', e);
    ...
    14: (initial) => {
        if (initial.main.networks.ethereum[137] && initial.main.networks.ethereum[137].connection) {
            ...
            if (primary.current === 'matic')
                primary.current = 'infura';
            if (secondary.current === 'matic')
                secondary.current = 'infura';
        }
    ...
    29: (initial) => {
        ...
        Object.entries(initial.main.accounts).forEach(([id, account]) => {
            ...
            const isPreviousDefaultWatchAccountName = account.lastSignerType.toLowerCase() === 'address' && account.name.toLowerCase() === 'address account';
            if (!isPreviousDefaultWatchAccountName && !(0, account_1.isDefaultAccountName)(account)) {
    ...
    36: (initial) => {
        if (initial?.main?.shortcuts?.summon &&
    ...
    37: (initial) => {
        const replaceAltGr = () => ((0, platform_1.isWindows)() ? ['Alt', 'Control'] : ['Alt']);
        ...
        const shortcutsSchema = zod_1.z
            .object({
            summon: zod_1.z.object({
                modifierKeys: zod_1.z.array(zod_1.z.string()),
                shortcutKey: zod_1.z.string(),
                enabled: zod_1.z.boolean(),
                configuring: zod_1.z.boolean()
            })
        })
        ...
        if (result.success) {
            ...
            electron_log_1.default.error('Migration 37: Could not migrate shortcuts', result.error);
        }
        return initial;
    }
};
...
const legacyMigrations = Object.entries(migrations).map(([version, legacyMigration]) => ({
    ...
    migrate: (initial) => legacyMigration(initial)
}));
exports.default = legacyMigrations;
